...
function M.flash_user(settings, env)
  return create_bear_append_command("flash-user", settings, env)
end
...
function M.compile_all(settings, env)
  ...
  table.insert(parts, "&&")
  table.insert(parts, "cd " .. env["appdir"])
  table.insert(parts, "&&")
...
function M.flash_all(settings, env)
  local parts = {}
  table.insert(parts, M.compile_all(settings, env))
  table.insert(parts, "&&")
  table.insert(parts, create_make_command("flash-all", settings, env))
  return table.concat(parts, " ")
end
